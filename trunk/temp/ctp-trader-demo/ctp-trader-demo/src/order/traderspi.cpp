...
void CtpTraderSpi::ReqUserLogin()
{
  
	CThostFtdcReqUserLoginField req;
	memset(&req, 0, sizeof(req));
	strcpy(req.BrokerID, "2030");
	strcpy(req.UserID,  "000000005510");
	strcpy(req.Password, "228636");
...
{ 	
    ...
    cerr<<" 响应 | 合约:"<<pInstrument->InstrumentID
		...
		<<" ProductClass:"<<pInstrument->ProductClass
			<<"Max Limit Vol:"<<pInstrument->MaxLimitOrderVolume
			<<"Min Limit Vol:"<<pInstrument->MinLimitOrderVolume
			<<"Factor:"<<pInstrument->VolumeMultiple
			<<"TickSize"<<pInstrument->PriceTick
		<<"交割年："<<pInstrument->DeliveryYear
...
int CtpTraderSpi::ReqQryExchange( )
{
	CThostFtdcQryExchangeField lquery;
	memset(&lquery,0,sizeof(lquery));
	strcpy(lquery.ExchangeID,"SHFE");

	int ret = pUserApi->ReqQryExchange(&lquery, ++requestId);
	cerr<< " 请求 | 发送撤单..." <<((ret == 0)?"成功":"失败") << endl;
	return 0;
}
...
void CtpTraderSpi::OnRtnInstrumentStatus( CThostFtdcInstrumentStatusField *pInstrumentStatus )
{
		...
		cerr<<" 状态改变 | 合约:"<< pInstrumentStatus->InstrumentID 
			<<" 状态:"<< pInstrumentStatus->InstrumentStatus
			<<" Enter time:"<<pInstrumentStatus->EnterTime
			<<" Enter reason:"<<pInstrumentStatus->EnterReason<<endl;
